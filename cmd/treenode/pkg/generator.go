...
func make_type_name(type_name string, suffix string, gv *GenericsValue) string {
	...
	generics_str := gv.String()
	builder.WriteString(generics_str)
...
func make_parameter_list(fields map[string]string) (string, error) {
	...
	for k, v := range fields {
		...
		uc.Assert(ok, "this should not happen")
	...
	for i := 0; i < len(field_list); i++ {
		builder.WriteString(field_list[i])
		builder.WriteRune(' ')
...
func make_assignment_list(fields map[string]string) (map[string]string, error) {
	...
	for i := 0; i < len(field_list); i++ {
		assignment_map[field_list[i]] = type_list[i]
	}
...
func (g *generator) Generate() ([]byte, error) {
	...
	tn_type_name := make_type_name(g.type_name, "", GenericsFlag)
	tn_iterator_name := make_type_name(g.type_name, "Iterator", GenericsFlag)
	...
	data := GenData{
		...
		TypeName:      tn_type_name,
		TypeSig:       tn_type_sig,
		VariableName:  g.variable_name,
		Fields:        g.fields,
		IteratorName:  tn_iterator_name,
...
func NewGenerator(type_name, var_name string) (*generator, error) {
...
// node is never nil.
func (iter {{ .IteratorSig }}) Consume() (tr.Noder, error) {
	if iter.current == nil {
...
func ({{ .VariableName }} {{ .TypeSig }}) Iterator() uc.Iterater[tr.Noder] {
	return &{{ .IteratorSig }}{
		parent: {{ .VariableName }},
...
func ({{ .VariableName }} {{ .TypeSig }}) FString(trav *ffs.Traversor, opts ...ffs.Option) error {
	if trav == nil {
		return nil
	}
...
func ({{ .VariableName }} {{ .TypeSig }}) SetParent(parent tr.Noder) bool {
	if parent == nil {
...
func ({{ .VariableName }} {{ .TypeSig }}) GetParent() tr.Noder {
...
func ({{ .VariableName }} {{ .TypeSig }}) LinkChildren(children []tr.Noder) {
	if len(children) == 0 {
		return
	}
...
func ({{ .VariableName }} {{ .TypeSig }}) GetLeaves() []tr.Noder {
...
//
// Finally, no nil nodes are returned.
func ({{ .VariableName }} {{ .TypeSig }}) GetAncestors() []tr.Noder {
...
func ({{ .VariableName }} {{ .TypeSig }}) GetFirstChild() tr.Noder {
...
func ({{ .VariableName }} {{ .TypeSig }}) DeleteChild(target tr.Noder) []tr.Noder {
	if target == nil {
		return nil
	}
...
func ({{ .VariableName }} {{ .TypeSig }}) AddChild(child tr.Noder) {
	if child == nil {
		return
	}
...
//	└── 5
//	└── 6
func ({{ .VariableName }} {{ .TypeSig }}) RemoveNode() []tr.Noder {
	prev := {{ .VariableName }}.PrevSibling
	next := {{ .VariableName }}.NextSibling
...
//   never nil.
func New{{ .TypeName }}({{ .ParamList }}) {{ .TypeSig }} {
	return &{{ .TypeSig }}{
...
func ({{ .VariableName }} {{ .TypeSig }}) GetChildren() []tr.Noder {
...
// Returns:
//   - []tr.Noder: A slice of pointers to the children of the node.
func ({{ .VariableName }} {{ .TypeSig }}) delete_child(target {{ .TypeSig }}) []tr.Noder {
...
//   - children: The children nodes. Must not be nil and must be of type {{ .TypeSig }}.
func delink_with_parent(parent {{ .TypeSig }}, children []tr.Noder) {
	if len(children) == 0 {
		return
	}
