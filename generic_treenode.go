...
// TreeNodeIterator[T any] is a pull-based iterator that iterates
// over the children of a TreeNode[T any].
...
// node is never nil.
func (iter *TreeNodeIterator[T]) Consume() (tr.Noder, error) {
	if iter.current == nil {
...
// TreeNode[T any] is a node in a tree.
...
func (tn *TreeNode[T]) GetLeaves() []tr.Noder {
	...
	stack := lls.NewLinkedStack[tr.Noder](tn)
...
func (tn *TreeNode[T]) DeleteChild(target tr.Noder) []tr.Noder {
	...
	delink_with_parent(tn, children)
...
func (tn *TreeNode[T]) RemoveNode() []tr.Noder {
	...
	delink_with_parent(tn, sub_roots)

	return sub_roots
}

// NewTreeNode[T any] creates a new node with the given data.
//
// Parameters:
//
//   - a: MyType[T]
...
// same as the behaviors of the TreeNode[T any].AddChild function.
//
// Parameters:
...
func (tn *TreeNode[T]) GetChildren() []tr.Noder {
...
// Returns:
//   - []tr.Noder: A slice of pointers to the children of the node.
func (tn *TreeNode[T]) delete_child(target *TreeNode[T]) []tr.Noder {
...
//   - children: The children nodes. Must not be nil and must be of type *TreeNode[T].
func delink_with_parent(parent *TreeNode[T], children []tr.Noder) {
	if len(children) == 0 {
		return
	}
